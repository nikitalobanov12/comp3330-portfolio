...
export default function ProjectPreviewCard({ count = 3 }: ProjectPreviewCardProps) {
  ...
  return (
    <section className="container mx-auto px-4 py-12">
      <div className="mb-8">
        <h2 className="text-3xl md:text-4xl font-bold font-[family-name:var(--font-roboto-mono)] mb-2">
          Featured Projects
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
        {displayedProjects.map((project) => (
          <Card key={project.title} className="flex flex-col h-full overflow-hidden hover:shadow-lg transition-shadow">
            <div className={`relative h-48 w-full ${project.color} flex items-center justify-center transition-transform hover:scale-105`}>
              <p className="text-white font-bold text-xl text-center px-4">
                {project.title}
              </p>
            </div>
            <CardHeader>
              <CardTitle className="font-[family-name:var(--font-roboto-mono)]">
                {project.title}
              </CardTitle>
            </CardHeader>
